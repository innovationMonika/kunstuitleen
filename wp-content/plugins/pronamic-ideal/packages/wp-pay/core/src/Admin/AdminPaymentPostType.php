...
class AdminPaymentPostType {
	...
	public function columns( $columns ) {
		$columns = [
			...
			'pronamic_payment_subscription' => sprintf(
				...
				esc_html__( 'Subscription', 'pronamic-ideal' ),
				esc_html__( 'Subscription', 'pronamic-ideal' )
			),
			'pronamic_payment_method'       => '',
			'pronamic_payment_title'        => __( 'Payment', 'pronamic-ideal' ),
			'pronamic_payment_transaction'  => __( 'Transaction', 'pronamic-ideal' ),
			'pronamic_payment_gateway'      => __( 'Gateway', 'pronamic-ideal' ),
			'pronamic_payment_description'  => __( 'Description', 'pronamic-ideal' ),
			'pronamic_payment_customer'     => __( 'Customer', 'pronamic-ideal' ),
			'pronamic_payment_amount'       => __( 'Amount', 'pronamic-ideal' ),
			'pronamic_payment_date'         => __( 'Date', 'pronamic-ideal' ),
		];
		...
		switch ( $column ) {
			...
			case 'pronamic_payment_subscription':
				...
				foreach ( $subscriptions as $subscription ) {
					$label = __( 'Recurring payment', 'pronamic-ideal' );
					$class = 'pronamic-pay-icon-recurring';

					if ( $subscription->is_first_payment( $payment ) ) {
						$label = __( 'First of recurring payment', 'pronamic-ideal' );
			...
			case 'pronamic_payment_amount':
				...
				if ( null !== $charged_back_amount ) {
					...
					$tip[] = \sprintf(
						/* translators: %s: formatted amount */
						__( '%s charged back', 'pronamic-ideal' ),
	...
	public function post_updated_messages( $messages ) {
		...
		$messages[ self::POST_TYPE ] = [
			...
			1  => __( 'Payment updated.', 'pronamic-ideal' ),
			...
			4  => __( 'Payment updated.', 'pronamic-ideal' ),
			...
			5  => isset( $_GET['revision'] ) ? sprintf( __( 'Payment restored to revision from %s.', 'pronamic-ideal' ), strval( wp_post_revision_title( (int) $_GET['revision'], false ) ) ) : false,
			...
			6  => __( 'Payment published.', 'pronamic-ideal' ),
			...
			7  => __( 'Payment saved.', 'pronamic-ideal' ),
			...
			8  => __( 'Payment submitted.', 'pronamic-ideal' ),
			...
			9  => sprintf( __( 'Payment scheduled for: %s.', 'pronamic-ideal' ), '<strong>' . $scheduled_date . '</strong>' ),
			...
			10 => __( 'Payment draft updated.', 'pronamic-ideal' ),
		];
