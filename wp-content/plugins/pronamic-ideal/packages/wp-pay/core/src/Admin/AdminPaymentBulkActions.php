...
class AdminPaymentBulkActions {
	...
	public function bulk_actions( $bulk_actions ) {
		...
		$bulk_actions['pronamic_payment_check_status'] = __( 'Check Payment Status', 'pronamic-ideal' );
	...
	public function admin_notices() {
		...
		if ( $updated > 0 ) {
			...
			$message = sprintf( _n( '%s payment updated.', '%s payments updated.', $updated, 'pronamic-ideal' ), number_format_i18n( $updated ) );
		...
		if ( $skipped > 0 ) {
			$message = sprintf(
				...
				_n( '%s payment is not updated because it already has a final payment status.', '%s payments are not updated because they already have a final payment status.', $skipped, 'pronamic-ideal' ),
		...
		if ( ! empty( $gateways ) ) {
			...
			$message = sprintf(
				...
				__( 'Requesting the current payment status is unsupported by %s.', 'pronamic-ideal' ),
				implode( ', ', $titles )
			);
