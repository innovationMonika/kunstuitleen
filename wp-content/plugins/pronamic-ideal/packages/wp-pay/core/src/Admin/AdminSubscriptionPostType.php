...
class AdminSubscriptionPostType {
			...
			\printf(
				...
				\wp_kses_post(
					\sprintf(
						...
						__( '%s has been created.', 'pronamic-ideal' ),
	...
	public function columns( $columns ) {
		$columns = [
			...
			'pronamic_subscription_status'       => sprintf(
				...
				esc_html__( 'Status', 'pronamic-ideal' ),
				esc_html__( 'Status', 'pronamic-ideal' )
			),
			'pronamic_subscription_method'       => '',
			'pronamic_subscription_title'        => __( 'Subscription', 'pronamic-ideal' ),
			'pronamic_subscription_customer'     => __( 'Customer', 'pronamic-ideal' ),
			'pronamic_subscription_amount'       => __( 'Amount', 'pronamic-ideal' ),
			'pronamic_subscription_recurring'    => __( 'Recurrence', 'pronamic-ideal' ),
			'pronamic_subscription_next_payment' => __( 'Next payment', 'pronamic-ideal' ),
			'pronamic_subscription_date'         => __( 'Date', 'pronamic-ideal' ),
		];
	...
	public function custom_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'pronamic_subscription_status':
				...
				$label = __( 'Unknown', 'pronamic-ideal' );
			...
			case 'pronamic_subscription_title':
				...
				echo wp_kses(
					sprintf(
						...
						__( '%1$s for %2$s %3$s', 'pronamic-ideal' ),
						$text,
						$source_description,
						$source_id_text
					),
	...
	public function post_updated_messages( $messages ) {
		...
		$messages[ self::POST_TYPE ] = [
			...
			1  => __( 'Subscription updated.', 'pronamic-ideal' ),
			...
			4  => __( 'Subscription updated.', 'pronamic-ideal' ),
			...
			5  => isset( $_GET['revision'] ) ? sprintf( __( 'Subscription restored to revision from %s.', 'pronamic-ideal' ), strval( wp_post_revision_title( (int) $_GET['revision'], false ) ) ) : false,
			...
			6  => __( 'Subscription published.', 'pronamic-ideal' ),
			...
			7  => __( 'Subscription saved.', 'pronamic-ideal' ),
			...
			8  => __( 'Subscription submitted.', 'pronamic-ideal' ),
			...
			9  => sprintf( __( 'Subscription scheduled for: %s.', 'pronamic-ideal' ), '<strong>' . $scheduled_date . '</strong>' ),
			...
			10 => __( 'Subscription draft updated.', 'pronamic-ideal' ),
		];
	...
	private function maybe_update_subscription() {
		...
		if ( ! \wp_verify_nonce( $nonce, 'pronamic_subscription_update' ) ) {
			\wp_die( \esc_html__( 'Action failed. Please refresh the page and retry.', 'pronamic-ideal' ) );
		}
			...
			if ( ! empty( $new_value ) && $old_value !== $new_value ) {
				...
				if ( false !== $updated_date ) {
					...
					$note = \sprintf(
						...
						\__( 'Next payment date updated from %1$s to %2$s.', 'pronamic-ideal' ),
						null === $next_payment_date ? '' : $next_payment_date->format_i18n( \__( 'D j M Y', 'pronamic-ideal' ) ),
						$updated_date->format_i18n( \__( 'D j M Y', 'pronamic-ideal' ) )
					);
