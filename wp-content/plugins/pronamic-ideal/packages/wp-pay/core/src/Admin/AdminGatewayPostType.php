...
class AdminGatewayPostType {
	...
	public function edit_columns( $columns ) {
		$columns = [
			...
			'pronamic_gateway_variant'   => __( 'Variant', 'pronamic-ideal' ),
			'pronamic_gateway_id'        => __( 'ID', 'pronamic-ideal' ),
			'pronamic_gateway_dashboard' => __( 'Dashboard', 'pronamic-ideal' ),
				...
				if ( isset( $integration ) ) {
					...
					if ( null !== $url ) {
						\printf(
							...
							esc_url( $url ),
							esc_html__( 'Dashboard', 'pronamic-ideal' )
						);
					}
				}
	...
	public function display_post_states( $post_states, $post ) {
		...
		if ( intval( get_option( 'pronamic_pay_config_id' ) ) === $post->ID ) {
			$post_states['pronamic_pay_config_default'] = __( 'Default', 'pronamic-ideal' );
		}
	...
	public function add_meta_boxes( $post_type ) {
		...
		add_meta_box(
			...
			__( 'Configuration', 'pronamic-ideal' ),
	...
	public function post_updated_messages( $messages ) {
		...
		$messages[ self::POST_TYPE ] = [
			...
			1  => __( 'Configuration updated.', 'pronamic-ideal' ),
			...
			4  => __( 'Configuration updated.', 'pronamic-ideal' ),
			...
			5  => isset( $_GET['revision'] ) ? sprintf( __( 'Configuration restored to revision from %s.', 'pronamic-ideal' ), strval( wp_post_revision_title( (int) $_GET['revision'], false ) ) ) : false,
			...
			6  => __( 'Configuration published.', 'pronamic-ideal' ),
			...
			7  => __( 'Configuration saved.', 'pronamic-ideal' ),
			...
			8  => __( 'Configuration submitted.', 'pronamic-ideal' ),
			...
			9  => sprintf( __( 'Configuration scheduled for: %s.', 'pronamic-ideal' ), '<strong>' . $scheduled_date . '</strong>' ),
			...
			10 => __( 'Configuration draft updated.', 'pronamic-ideal' ),
		];
