...
class AdminHealth {
	...
	public function debug_information( $debug_information ) {
		...
		// Time.
		$fields['time'] = [
			'label' => __( 'Time (UTC)', 'pronamic-ideal' ),
			'value' => esc_html( gmdate( __( 'Y/m/d g:i:s A', 'pronamic-ideal' ) ) ),
		];

		// OpenSSL version.
		$openssl_version = __( 'Not available', 'pronamic-ideal' );
	...
	public function test_wordpress_version() {
		...
		$result = [
			...
			'label'       => sprintf(
				...
				__( 'WordPress version is supported by Pronamic Pay (%s)', 'pronamic-ideal' ),
				get_bloginfo( 'version' )
			),
			'description' => sprintf( '<p>%s</p>', __( 'Pronamic Pay requires at least WordPress 4.7.', 'pronamic-ideal' ) ),
			'badge'       => [
				'label' => __( 'Payments', 'pronamic-ideal' ),
				'color' => 'blue',
			],
			'status'      => 'good',
			'actions'     => '',
		...
		if ( version_compare( get_bloginfo( 'version' ), '4.7', '<' ) ) {
			...
			$result['label']  = __( 'Pronamic Pay requires at least WordPress 4.7', 'pronamic-ideal' );
		}
	...
	public function test_memory_limit() {
		...
		$result = [
			...
			'label'       => sprintf(
				...
				__( 'WordPress memory limit is sufficient (%s)', 'pronamic-ideal' ),
				size_format( $memory )
			),
			'description' => sprintf( '<p>%s</p>', __( 'Pronamic Pay recommends setting the WordPress memory limit to at least 64 MB.', 'pronamic-ideal' ) ),
			'badge'       => [
				'label' => __( 'Payments', 'pronamic-ideal' ),
				'color' => 'blue',
			],
		...
		if ( $memory < 67108864 ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: WordPress memory limit */
				__( 'Increase WordPress memory limit (%s) to at least 64 MB', 'pronamic-ideal' ),
				size_format( $memory )
			);
		}
	...
	public function test_character_set() {
		...
		$result = [
			...
			'label'       => __( 'Character encoding is set to UTF-8', 'pronamic-ideal' ),
			'description' => sprintf( '<p>%s</p>', __( 'Pronamic Pay recommends to use the UTF-8 character encoding for payments.', 'pronamic-ideal' ) ),
			'badge'       => [
				'label' => __( 'Payments', 'pronamic-ideal' ),
				'color' => 'blue',
			],
		...
		if ( 0 !== strcasecmp( get_bloginfo( 'charset' ), 'UTF-8' ) ) {
			...
			$result['label'] = __( 'Character encoding is not set to UTF-8', 'pronamic-ideal' );
		}
	...
	public function test_hashing_algorithms() {
		...
		$result = [
			...
			'label'       => __( 'SHA1 hashing algorithm is available', 'pronamic-ideal' ),
			'description' => sprintf( '<p>%s</p>', __( 'Payment gateways often use the SHA1 hashing algorithm, therefore Pronamic Pay advises to enable this hashing algorithm.', 'pronamic-ideal' ) ),
			'badge'       => [
				'label' => __( 'Payments', 'pronamic-ideal' ),
				'color' => 'blue',
			],
		...
		if ( ! in_array( 'sha1', $algorithms, true ) ) {
			...
			$result['label'] = __( 'SHA1 hashing algorithm is not available for Pronamic Pay', 'pronamic-ideal' );
		}
	...
	public function test_extensions_support() {
		...
		if ( 0 !== count( $untested_plugin_versions ) ) {
			...
			$extensions_list_text .= sprintf(
				'<p>%1$s</p><p>%2$s</p>',
				__( 'Untested plugin versions:', 'pronamic-ideal' ),
		...
		if ( 0 !== count( $outdated_plugin_versions ) ) {
			...
			$extensions_list_text .= sprintf(
				'<p>%1$s</p><p>%2$s</p>',
				__( 'Outdated unsupported plugin versions for which payments can not be processed:', 'pronamic-ideal' ),
		...
		if ( 0 !== count( $supported_extensions ) ) {
			$extensions_list_text .= sprintf(
				'<p>%1$s</p><p>%2$s</p>',
				__( 'Supported plugin versions:', 'pronamic-ideal' ),
		...
		$label = sprintf(
			...
			__( '%s extensions are compatible', 'pronamic-ideal' ),
			__( 'Pronamic Pay', 'pronamic-ideal' )
		);

		$description_text = __( 'Pronamic Pay uses extensions to integrate with form, booking and other e-commerce plugins. All extensions support the currently activated plugin versions.', 'pronamic-ideal' );

		if ( 'good' !== $status ) {
			$label = sprintf(
				/* translators: %s: plugin name */
				__( '%s extensions are incompatible', 'pronamic-ideal' ),
				__( 'Pronamic Pay', 'pronamic-ideal' )
			);
