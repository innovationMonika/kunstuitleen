...
class Plugin {
	...
	public function __construct( $args = [] ) {
		...
		$payment_method_afterpay_nl->descriptions = [
			 ...
			 */
			'default' => \__( 'AfterPay is one of the largest and most popular post-payment system in the Benelux. Millions of Dutch and Belgians use AfterPay to pay for products.', 'pronamic-ideal' ),
		];
		...
		$description_template = \__( 'By using this payment method you authorize us via %s to debit payments from your bank account.', 'pronamic-ideal' );
		...
		$payment_method_ideal->descriptions = [
			'customer' => \__( 'With iDEAL you can easily pay online in the secure environment of your own bank.', 'pronamic-ideal' ),
		];
		...
		$payment_method_riverty->descriptions = [
			'default'  => \__( 'Riverty (formerly AfterPay) is a payment service that allows customers to pay after receiving the product.', 'pronamic-ideal' ),
	...
	public static function get_default_error_message() {
		return __( 'Something went wrong with the payment. Please try again or pay another way.', 'pronamic-ideal' );
	}
	...
	public static function get_config_select_options( $payment_method = null ) {
		...
		$options = [ __( '— Select Configuration —', 'pronamic-ideal' ) ];
		...
		if ( null === $gateway ) {
			$payment->add_note(
				\sprintf(
					...
					\__( 'Payment failed because gateway configuration with ID `%d` does not exist.', 'pronamic-ideal' ),
					$config_id
				)
			);
	...
	public static function create_refund( Refund $refund ) {
		...
		if ( null === $gateway ) {
			throw new \Exception(
				\esc_html__( 'Unable to process refund as gateway could not be found.', 'pronamic-ideal' )
			);
		}
