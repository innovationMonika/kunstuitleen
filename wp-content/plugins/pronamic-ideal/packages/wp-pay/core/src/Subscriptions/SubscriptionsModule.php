...
class SubscriptionsModule {
	...
	private function get_subscription_status_update_note( $old_status, $new_status ) {
		...
		if ( null === $old_status ) {
			...
			/* translators: 1: new status */
				__( 'Subscription created with status "%1$s".', 'pronamic-ideal' ),
		...
		/* translators: 1: old status, 2: new status */
			__( 'Subscription status changed from "%1$s" to "%2$s".', 'pronamic-ideal' ),
			...
			if ( ! empty( $mandate_id ) ) {
				try {
					if ( ! \is_callable( [ $gateway, 'update_subscription_mandate' ] ) ) {
						throw new \Exception( __( 'Gateway does not support subscription mandate updates.', 'pronamic-ideal' ) );
					}
		...
		\register_rest_route(
			...
			[
				...
				'args'                => [
					...
					'sequence_number' => [
						'description' => __( 'Subscription phase sequence number.', 'pronamic-ideal' ),
						'type'        => 'integer',
					],
				],
			]
		...
		if ( null === $subscription ) {
			return new \WP_Error(
				...
				\sprintf(
					...
					\__( 'Could not find subscription with ID `%s`.', 'pronamic-ideal' ),
					$subscription_id
				),
				$subscription_id
			);
	...
	public function rest_api_subscription_phase( \WP_REST_Request $request ) {
		...
		if ( null === $phase ) {
			return new \WP_Error(
				...
				\sprintf(
					/* translators: %s: Subscription ID */
					\__( 'Could not find subscription phase with sequence number `%s`.', 'pronamic-ideal' ),
					$sequence_number
				),
				$sequence_number
			);
	...
	public function source_text_subscription_payment_method_change( $text ) {
		$text = \__( 'Subscription payment method change', 'pronamic-ideal' );
