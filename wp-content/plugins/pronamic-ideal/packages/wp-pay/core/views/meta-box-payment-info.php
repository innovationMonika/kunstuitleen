...
<table class="form-table">
	...
	if ( PaymentStatus::FAILURE === $payment->get_status() && null !== $failure_reason ) :
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Status', 'pronamic-ideal' ); ?>
			</th>
			<td>
				<?php
	...
	if ( $amount_refunded->get_value() > 0 ) :
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Refunded Amount', 'pronamic-ideal' ); ?>
			</th>
			<td>
	...
	if ( null !== $charged_back_amount && $charged_back_amount->get_value() > 0 ) :
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Charged Back Amount', 'pronamic-ideal' ); ?>
			</th>
			<td>
	...
	<tr>
		<th scope="row">
			<?php esc_html_e( 'Transaction ID', 'pronamic-ideal' ); ?>
		</th>
		<td>
			<?php
	...
	if ( $purchase_id ) :
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Purchase ID', 'pronamic-ideal' ); ?>
			</th>
			<td>
	...
	if ( null !== $consumer_bank_details ) :
		...
		if ( null !== $consumer_name ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'Account Holder', 'pronamic-ideal' ); ?>
				</th>
				<td>
		...
		if ( null !== $account_holder_city ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'Account Holder City', 'pronamic-ideal' ); ?>
				</th>
				<td>
		...
		if ( null !== $account_holder_country ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'Account Holder Country', 'pronamic-ideal' ); ?>
				</th>
				<td>
		...
		if ( null !== $account_number ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'Account Number', 'pronamic-ideal' ); ?>
				</th>
				<td>
		...
		if ( null !== $iban ) :
			...
			<tr>
				<th scope="row">
					...
					printf(
						'<abbr title="%s">%s</abbr>',
						esc_attr( _x( 'International Bank Account Number', 'IBAN abbreviation title', 'pronamic-ideal' ) ),
						esc_html__( 'IBAN', 'pronamic-ideal' )
					);
		...
		if ( null !== $bic ) :
			...
			<tr>
				<th scope="row">
					...
					printf(
						'<abbr title="%s">%s</abbr>',
						esc_attr( _x( 'Bank Identifier Code', 'BIC abbreviation title', 'pronamic-ideal' ) ),
						esc_html__( 'BIC', 'pronamic-ideal' )
					);
	...
	<?php if ( null !== $bank_transfer_recipient ) : ?>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Bank Transfer Recipient', 'pronamic-ideal' ); ?>
			</th>
			<td>
				<?php
	...
	if ( null !== $customer ) :
		...
		if ( null !== $company_name ) :
			...
			<tr>
				<th scope="row">
					<?php \esc_html_e( 'Company', 'pronamic-ideal' ); ?>
				</th>
				<td>
		...
		if ( null !== $vat_number ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'VAT Number', 'pronamic-ideal' ); ?>
				</th>
				<td>
			...
			<?php if ( null !== $vat_number_validity ) : ?>
				...
				<tr>
					<th scope="row">
						<?php esc_html_e( 'VAT Number Validity', 'pronamic-ideal' ); ?>
					</th>
					<td style="padding-top: 20px">
						<table class="pronamic-pay-form-sub-table">
							...
							<tr>
								<th scope="row">
									<?php esc_html_e( 'Request Date', 'pronamic-ideal' ); ?>
								</th>
								<td>
							...
							<tr>
								<th scope="row">
									<?php esc_html_e( 'Valid', 'pronamic-ideal' ); ?>
								</th>
								<td>
									<?php echo esc_html( $vat_number_validity->is_valid() ? __( 'Yes', 'pronamic-ideal' ) : __( 'No', 'pronamic-ideal' ) ); ?>
								</td>
							</tr>
							<tr>
								<th scope="row">
									<?php esc_html_e( 'Name', 'pronamic-ideal' ); ?>
								</th>
								<td>
							...
							<tr>
								<th scope="row">
									<?php esc_html_e( 'Address', 'pronamic-ideal' ); ?>
								</th>
								<td>
									<?php
							...
							if ( null !== $service ) :
								...
								<tr>
									<th scope="row">
										<?php esc_html_e( 'Validation Service', 'pronamic-ideal' ); ?>
									</th>
									<td>
										<?php
										...
										switch ( $service ) {
											case VatNumberValidationService::VIES:
												echo esc_html( __( 'VIES', 'pronamic-ideal' ) );
		...
		if ( null !== $birth_date ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'Date of birth', 'pronamic-ideal' ); ?>
				</th>
				<td>
					<?php

					echo esc_html( $birth_date->format_i18n( __( 'D j M Y', 'pronamic-ideal' ) ) )
		...
		if ( null !== $gender ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'Gender', 'pronamic-ideal' ); ?>
				</th>
				<td>
					<?php
					...
					switch ( $gender ) {
						case Gender::FEMALE:
							echo esc_html( __( 'Female', 'pronamic-ideal' ) );

							break;
						case Gender::MALE:
							echo esc_html( __( 'Male', 'pronamic-ideal' ) );

							break;
						case Gender::OTHER:
							echo esc_html( __( 'Other', 'pronamic-ideal' ) );
		...
		if ( null !== $ip_address ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'IP Address', 'pronamic-ideal' ); ?>
				</th>
				<td>
	...
	<?php if ( null !== $payment->get_billing_address() ) : ?>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Billing Address', 'pronamic-ideal' ); ?>
			</th>
			<td>
				<?php
	...
	<?php if ( null !== $payment->get_shipping_address() ) : ?>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Shipping Address', 'pronamic-ideal' ); ?>
			</th>
			<td>
				<?php
	...
	<?php if ( 'membership' === $payment->get_source() ) : ?>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'User ID', 'pronamic-ideal' ); ?>
			</th>
			<td>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Subscription ID', 'pronamic-ideal' ); ?>
			</th>
			<td>
	...
	<?php if ( ! empty( $ogone_alias ) && \is_string( $ogone_alias ) ) : ?>
		...
		<tr>
			<th scope="row">
				<?php esc_html_e( 'Ingenico Alias', 'pronamic-ideal' ); ?>
			</th>
			<td>
	...
	<?php if ( $plugin->is_debug_mode() ) : ?>
		...
		<?php if ( null !== $payment->get_version() ) : ?>
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'Version', 'pronamic-ideal' ); ?>
				</th>
				<td>
	...
	if ( null !== $customer ) :
		...
		if ( null !== $user_agent ) :
			...
			<tr>
				<th scope="row">
					<?php esc_html_e( 'User Agent', 'pronamic-ideal' ); ?>
				</th>
				<td>
	...
	<tr>
		<th scope="row">
			<?php esc_html_e( 'Action URL', 'pronamic-ideal' ); ?>
		</th>
		<td>
			<?php
	...
	<tr>
		<th scope="row">
			<?php esc_html_e( 'Pay Redirect URL', 'pronamic-ideal' ); ?>
		</th>
		<td>
			<?php
	...
	<tr>
		<th scope="row">
			<?php esc_html_e( 'Return URL', 'pronamic-ideal' ); ?>
		</th>
		<td>
			<?php
	...
	<tr>
		<th scope="row">
			<?php esc_html_e( 'Return Redirect URL', 'pronamic-ideal' ); ?>
		</th>
		<td>
			<?php
