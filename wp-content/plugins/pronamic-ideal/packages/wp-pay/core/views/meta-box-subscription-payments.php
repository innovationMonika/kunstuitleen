...
if ( 0 === count( $payments ) ) : ?>
	...
	<?php esc_html_e( 'No payments found.', 'pronamic-ideal' ); ?>
...
<?php else : ?>
	...
	<table class="pronamic-pay-table widefat">
		<thead>
			<tr>
				<th scope="col">
					<span class="pronamic-pay-tip pronamic-pay-icon pronamic-pay-status" title="<?php esc_attr_e( 'Status', 'pronamic-ideal' ); ?>"><?php esc_html_e( 'Status', 'pronamic-ideal' ); ?></span>
				</th>
				<th scope="col"><?php esc_html_e( 'Period', 'pronamic-ideal' ); ?></th>
				<th scope="col"><?php esc_html_e( 'Payment', 'pronamic-ideal' ); ?></th>
				<th scope="col"><?php esc_html_e( 'Transaction', 'pronamic-ideal' ); ?></th>
						...
						if ( in_array( $subscription->get_source(), [ 'woocommerce' ], true ) && null !== $next_payment_date ) :
							...
							echo wp_kses_post(
								sprintf(
									...
									__( 'Will be created on %s', 'pronamic-ideal' ),
									\esc_html( $next_payment_date->format_i18n( __( 'D j M Y', 'pronamic-ideal' ) ) )
								)
							);
						...
						elseif ( null !== $next_payment_delivery_date ) :
							...
							echo wp_kses_post(
								sprintf(
									...
									__( 'Will be created on %1$s or %2$s', 'pronamic-ideal' ),
									\esc_html( $next_payment_delivery_date->format_i18n( __( 'D j M Y', 'pronamic-ideal' ) ) ),
									\sprintf(
										'<a href="%1$s">%2$s</a>',
										\esc_url( $create_next_payment_url ),
										\esc_html( \__( 'create now', 'pronamic-ideal' ) )
									)
								)
							);
								...
								\printf(
									...
									\esc_html(
										sprintf(
											...
											__( 'Retry payment #%d', 'pronamic-ideal' ),
											$payment_id
										)
									)
								);
