...
class FormProcessor {
	...
	public function init() {
		...
		if ( empty( $description ) ) {
			$description = sprintf(
				...
				__( 'Payment Form %s', 'pronamic-ideal' ),
				$order_id
			);
		}
	...
	private function validate() {
		...
		try {
			...
			$pronamic_pay_errors['amount'] = __( 'Please enter a valid amount', 'pronamic-ideal' );
		}
		...
		if ( \array_key_exists( 'pronamic_pay_first_name', $_POST ) ) {
			...
			if ( empty( $first_name ) ) {
				$pronamic_pay_errors['first_name'] = __( 'Please enter your first name', 'pronamic-ideal' );
			}
		}
		...
		if ( empty( $email ) ) {
			$pronamic_pay_errors['email'] = __( 'Please enter a valid email address', 'pronamic-ideal' );
		}
