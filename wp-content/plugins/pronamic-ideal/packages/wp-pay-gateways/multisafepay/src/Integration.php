...
class Integration extends AbstractGatewayIntegration {
	...
	public function __construct( $args = [] ) {
		$args = wp_parse_args(
			...
			[
				...
				'product_url'   => \__( 'http://www.multisafepay.com/', 'pronamic-ideal' ),
				...
				'manual_url'    => \__( 'https://www.pronamicpay.com/en/manuals/how-to-connect-multisafepay-to-wordpress-with-pronamic-pay/', 'pronamic-ideal' ),
			]
		);
	...
	public function get_settings_fields() {
		...
		$fields[] = [
			...
			'title'    => __( 'Account ID', 'pronamic-ideal' ),
			'type'     => 'text',
			'classes'  => [ 'code' ],
			'tooltip'  => sprintf(
				'%s %s.',
				__( 'Account ID', 'pronamic-ideal' ),
				/* translators: %s: payment provider name */
				sprintf( __( 'as mentioned in the %s dashboard', 'pronamic-ideal' ), __( 'MultiSafepay', 'pronamic-ideal' ) )
			),
		];
		...
		$fields[] = [
			...
			'title'    => __( 'Site ID', 'pronamic-ideal' ),
			'type'     => 'text',
			'classes'  => [ 'code' ],
			'tooltip'  => sprintf(
				'%s %s.',
				__( 'Site ID', 'pronamic-ideal' ),
				/* translators: %s: payment provider name */
				sprintf( __( 'as mentioned in the %s dashboard', 'pronamic-ideal' ), __( 'MultiSafepay', 'pronamic-ideal' ) )
			),
		];
		...
		$fields[] = [
			...
			'title'    => __( 'Site Security Code', 'pronamic-ideal' ),
			'type'     => 'text',
			'classes'  => [ 'code' ],
			'tooltip'  => sprintf(
				'%s %s.',
				__( 'Site Security Code', 'pronamic-ideal' ),
				/* translators: %s: payment provider name */
				sprintf( __( 'as mentioned in the %s dashboard', 'pronamic-ideal' ), __( 'MultiSafepay', 'pronamic-ideal' ) )
			),
		];
