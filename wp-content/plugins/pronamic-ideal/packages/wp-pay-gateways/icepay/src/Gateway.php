...
class Gateway extends Core_Gateway {
	...
	public function __construct( Config $config ) {
		...
		$credit_card_issuer_field->set_label( __( 'Brand', 'pronamic-ideal' ) );
	...
	private function get_credit_card_issuers() {
		...
		foreach ( $issuers as $issuer ) {
			switch ( $issuer ) {
				case 'AMEX':
					$name = _x( 'AMEX', 'Payment method name', 'pronamic-ideal' );

					break;
				case 'MASTER':
					$name = _x( 'MASTER', 'Payment method name', 'pronamic-ideal' );

					break;
				case 'VISA':
					$name = _x( 'VISA', 'Payment method name', 'pronamic-ideal' );
