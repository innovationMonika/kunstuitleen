	...
	public function rest_api_init() {
		...
		\register_rest_route(
			...
			[
				'args'                => [
					'id' => [
						'description' => \__( 'Unique identifier for the gateway configuration post.', 'pronamic-ideal' ),
						'type'        => 'integer',
					],
				],
	...
	public function rest_api_omnikassa_2_webhook_item( WP_REST_Request $request ) {
		...
		try {
			...
			return new \WP_Error(
				...
				\__( 'Invalid Rabo Smart Pay notification.', 'pronamic-ideal' ),
				[
					'notification' => $json,
					'status'       => 400,
				]
		...
		if ( ! \is_numeric( $id ) ) {
			return new \WP_Error(
				...
				\__( 'No gateway ID given in `id` parameter.', 'pronamic-ideal' )
			);
		}
		...
		if ( ! $gateway instanceof Gateway ) {
			...
			return new \WP_Error(
				...
				\__( 'Invalid Rabo Smart Pay gateway.', 'pronamic-ideal' ),
				[
					'id'     => $id,
					'status' => 400,
				]
