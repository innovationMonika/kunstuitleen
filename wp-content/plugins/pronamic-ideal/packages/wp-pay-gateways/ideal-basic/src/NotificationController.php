	...
	public function rest_api_notification( \WP_REST_Request $request ) {
		...
		if ( empty( $body ) ) {
			return new \WP_Error(
				...
				__( 'The iDEAL Basic notification is empty.', 'pronamic-ideal' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( false === $simplexml ) {
			return new \WP_Error(
				...
				\__( 'The PHP SimpleXML extension could not interpret the XML.', 'pronamic-ideal' ),
				[
					'status' => 404,
					'xml'    => $body,
				]
		...
		if ( null === $payment ) {
			return new \WP_Error(
				...
				\sprintf(
					...
					__( 'Could not find iDEAL Basic payment by purchase ID: %s.', 'pronamic-ideal' ),
					$purchase_id
				),
				[ 'status' => 404 ]
			);
		...
		$note = sprintf(
			...
			__( 'Webhook requested by %s.', 'pronamic-ideal' ),
			__( 'iDEAL Basic', 'pronamic-ideal' )
		);
