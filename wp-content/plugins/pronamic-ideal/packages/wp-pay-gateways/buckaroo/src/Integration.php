...
class Integration extends AbstractGatewayIntegration {
	...
	public function __construct( $args = [] ) {
		$args = wp_parse_args(
			...
			[
				...
				'product_url'          => \__( 'http://www.buckaroo-payments.com', 'pronamic-ideal' ),
				...
				'manual_url'           => \__( 'https://www.pronamicpay.com/en/manuals/how-to-connect-buckaroo-to-wordpress-with-pronamic-pay/', 'pronamic-ideal' ),
	...
	public function get_settings_fields() {
		...
		$fields[] = [
			...
			'title'    => __( 'Website Key', 'pronamic-ideal' ),
			'type'     => 'text',
			'classes'  => [ 'code' ],
			'tooltip'  => __( 'Website key as mentioned in the Buckaroo dashboard on the page "Profile » Website".', 'pronamic-ideal' ),
		...
		$fields[] = [
			...
			'tooltip'  => __( 'Secret key as mentioned in the Buckaroo dashboard on the page "Configuration » Secret Key for Digital Signature".', 'pronamic-ideal' ),
		...
		$fields[] = [
			...
			'title'    => __( 'Excluded services', 'pronamic-ideal' ),
			'type'     => 'text',
			'classes'  => [ 'regular-text', 'code' ],
			'tooltip'  => sprintf(
				/* translators: %s: <code>brq_parameter</code> */
				__( 'This controls the Buckaroo %s parameter.', 'pronamic-ideal' ),
		...
		$fields[] = [
			...
			'title'       => __( 'Invoice number', 'pronamic-ideal' ),
			'type'        => 'text',
			'classes'     => [ 'regular-text', 'code' ],
			'tooltip'     => sprintf(
				/* translators: %s: <code>brq_parameter</code> */
			...
			'description' => sprintf(
				...
				sprintf( __( 'Available tags: %s', 'pronamic-ideal' ), sprintf( '<code>%s</code> <code>%s</code>', '{order_id}', '{payment_id}' ) ),
				/* translators: %s: default code */
				sprintf( __( 'Default: <code>%s</code>', 'pronamic-ideal' ), '{payment_id}' )
			),
		];
		...
		$fields[] = [
			'section'  => 'feedback',
			'title'    => __( 'Push URL', 'pronamic-ideal' ),
			'type'     => 'text',
			'classes'  => [ 'large-text', 'code' ],
			'value'    => \rest_url( self::REST_ROUTE_NAMESPACE . '/push' ),
			'readonly' => true,
			'tooltip'  => __( 'The Push URL as sent with each transaction to receive automatic payment status updates on.', 'pronamic-ideal' ),
		];
