			...
			if ( PaymentStatus::FAILURE === $status ) {
				throw new \Exception(
					\sprintf(
						...
						__( 'Unable to create payment at gateway: %1$s%2$s', 'pronamic-ideal' ),
	...
	public function create_refund( Refund $refund ) {
		...
		if ( ! \is_object( $original_transaction ) ) {
			throw new \Exception(
				sprintf(
					...
					__( 'Unable to create refund for transaction with transaction key: %s', 'pronamic-ideal' ),
					$transaction_id
				)
			);
		}
		...
		if ( null === $service_name ) {
			throw new \Exception(
				sprintf(
					/* translators: %s: transaction key */
					__( 'Unable to create refund for transaction without service name. Transaction key: %s', 'pronamic-ideal' ),
					$transaction_id
				)
			);
		}
		...
		if ( \property_exists( $result, 'Status' ) && \property_exists( $result->Status, 'Code' ) ) {
			...
			if ( PaymentStatus::SUCCESS !== $status ) {
				throw new \Exception(
					\sprintf(
						/* translators: 1: payment provider name, 2: status message, 3: status sub message*/
						__( 'Unable to create refund at %1$s gateway: %2$s%3$s', 'pronamic-ideal' ),
						__( 'Buckaroo', 'pronamic-ideal' ),
