...
class Client {
	...
	private function send_request( $version, $namespace, $method, $output, $parameters = [] ) {
		...
		if ( ! is_object( $result ) ) {
			throw new \Exception( __( 'Unknown response from Pay.nl error.', 'pronamic-ideal' ) );
		}
		...
		if ( isset( $result->request, $result->request->result ) && '1' !== $result->request->result ) {
			throw new \Exception( __( 'Unknown Pay.nl error.', 'pronamic-ideal' ) );
		}
	...
	public function get_issuers() {
		...
		if ( ! \is_array( $data ) ) {
			throw new \Exception( \__( 'Failed to request banks from Pay., received an unexpected answer from Pay.', 'pronamic-ideal' ) );
		}
