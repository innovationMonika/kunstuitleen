	...
	private function save_action_to_db( ActionScheduler_Action $action, DateTime $date = null, $unique = false ) {
		...
		try {
			...
			if ( is_wp_error( $action_id ) ) {
				...
				throw new \RuntimeException( $wpdb->last_error ? $wpdb->last_error : __( 'Database error.', 'pronamic-ideal' ) );
			}
			...
			throw new \RuntimeException( sprintf( __( 'Error saving action: %s', 'pronamic-ideal' ), $e->getMessage() ), 0 );
		}
	}
...
",
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, array( 'select', 'count' ), true ) ) {
			throw new InvalidArgumentException( __( 'Invalid value for select or count parameter. Cannot query actions.', 'pronamic-ideal' ) );
		}
		...
		if ( ! is_null( $query['args'] ) ) {
			switch ( $query['partial_args_matching'] ) {
				case 'json':
					if ( ! $supports_json ) {
						throw new \RuntimeException( __( 'JSON partial matching not supported in your environment. Please check your MySQL/MariaDB version.', 'pronamic-ideal' ) );
					}
				...
				case 'off':
					...
					break;
				default:
					throw new \RuntimeException( __( 'Unknown partial args matching value.', 'pronamic-ideal' ) );
			}
		}
	...
	public function cancel_action( $action_id ) {
		...
		if ( false === $updated ) {
			...
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s: we were unable to cancel this action. It may may have been deleted by another process.', 'pronamic-ideal' ), $action_id ) );
		}
	...
	public function delete_action( $action_id ) {
		...
		if ( empty( $deleted ) ) {
			...
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s: we were unable to delete this action. It may may have been deleted by another process.', 'pronamic-ideal' ), $action_id ) );
		}
	...
	protected function get_date_gmt( $action_id ) {
		...
		if ( empty( $record ) ) {
			/* translators: %s is the action ID */
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s: we were unable to determine the date of this action. It may may have been deleted by another process.', 'pronamic-ideal' ), $action_id ) );
		}
	...
	protected function claim_actions( $claim_id, $limit, \DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( false === $rows_affected ) {
			$error = empty( $wpdb->last_error )
				? _x( 'unknown', 'database error', 'pronamic-ideal' )
				: $wpdb->last_error;

			throw new \RuntimeException(
				sprintf(
					/* translators: %s database error. */
					__( 'Unable to claim actions. Database error: %s.', 'pronamic-ideal' ),
					$error
				)
			);
		}
		...
		if ( $row_updates < count( $action_ids ) ) {
			throw new RuntimeException(
				sprintf(
					...
					__( 'Unable to release actions from claim id %d.', 'pronamic-ideal' ),
					$claim->get_id()
				)
			);
		}
	...
	public function mark_failure( $action_id ) {
		...
		if ( empty( $updated ) ) {
			/* translators: %s is the action ID */
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s: we were unable to mark this action as having failed. It may may have been deleted by another process.', 'pronamic-ideal' ), $action_id ) );
		}
	}
		...
		if ( ! $status_updated ) {
			throw new Exception(
				sprintf(
					...
					__( 'Unable to update the status of action %1$d to %2$s.', 'pronamic-ideal' ),
					$action_id,
					self::STATUS_RUNNING
				)
			);
	...
	public function mark_complete( $action_id ) {
		...
		if ( empty( $updated ) ) {
			/* translators: %s is the action ID */
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s: we were unable to mark this action as having completed. It may may have been deleted by another process.', 'pronamic-ideal' ), $action_id ) );
		}
	...
	public function get_status( $action_id ) {
		...
		if ( null === $status ) {
			throw new \InvalidArgumentException( __( 'Invalid action ID. No status found.', 'pronamic-ideal' ) );
		} elseif ( empty( $status ) ) {
			throw new \RuntimeException( __( 'Unknown status found for action.', 'pronamic-ideal' ) );
		} else {
			return $status;
		}
	}
