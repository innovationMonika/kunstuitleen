...
class Extension extends AbstractPluginIntegration {
	...
	public function source_text( $text, Payment $payment ) {
		$text = __( 'Gravity Forms', 'pronamic-ideal' ) . '<br />';

		$text .= sprintf(
			'<a href="%1$s">%2$s</a>',
			add_query_arg( [ 'pronamic_gf_lid' => $payment->get_source_id() ], admin_url( 'admin.php' ) ),
			/* translators: %s: source id  */
			sprintf( __( 'Entry #%s', 'pronamic-ideal' ), $payment->get_source_id() )
		);
	...
	public function source_description( $description, Payment $payment ) {
		return __( 'Gravity Forms Entry', 'pronamic-ideal' );
	}
	...
	public function subscription_source_text( $text, Subscription $subscription ) {
		...
		$text .= sprintf(
			...
			/* translators: %s: source id  */
			sprintf( __( 'Entry #%s', 'pronamic-ideal' ), $subscription->get_source_id() )
		);
		...
		$result = $this->addon->refund_payment(
			...
			[
				...
				'note'           => \sprintf(
					...
					\__( 'Payment has been (partially) refunded. Amount: %s.', 'pronamic-ideal' ),
		...
		switch ( $subscription->get_status() ) {
			case SubscriptionStatus::ACTIVE:
				if ( ! Entry::is_payment_active( $lead ) ) {
					$action['note'] = __( 'Subscription manually activated.', 'pronamic-ideal' );
		...
		switch ( $action['type'] ) {
			...
			case 'cancel_subscription':
				...
				if ( ! isset( $action['note'] ) ) {
					$action['note'] = __( 'Subscription manually canceled.', 'pronamic-ideal' );
				}
	...
	public static function get_delay_actions() {
		$actions = [
			'gravityformsactivecampaign'   => [
				...
				'label'                       => __( 'Subscribing the user to ActiveCampaign', 'pronamic-ideal' ),
			],
			'gravityformsaweber'           => [
				'active'                      => false,
				'meta_key_suffix'             => 'aweber_subscription',
				'delayed_payment_integration' => true,
				'label'                       => __( 'Subscribing the user to AWeber', 'pronamic-ideal' ),
			...
			'gravityformscampaignmonitor'  => [
				...
				'delayed_payment_integration' => true,
				'label'                       => __( 'Subscribing the user to Campaign Monitor', 'pronamic-ideal' ),
				'delay_callback'              => function () {
			...
			'gravityformsmailchimp'        => [
				...
				'delayed_payment_integration' => true,
				'label'                       => __( 'Subscribing the user to MailChimp', 'pronamic-ideal' ),
				'delay_callback'              => function () {
			...
			'slicedinvoices'               => [
				...
				'label'                       => __( 'Creating quotes and invoices with Sliced Invoices', 'pronamic-ideal' ),
			],
			'gravityforms-moneybird'       => [
				'active'                      => false,
				'meta_key_suffix'             => 'moneybird',
				'delayed_payment_integration' => false,
				'label'                       => __( 'Sending estimates and invoices with Moneybird', 'pronamic-ideal' ),
			],
			'gravityformstwilio'           => [
				'active'                      => false,
				'meta_key_suffix'             => 'twilio',
				'delayed_payment_integration' => true,
				'label'                       => __( 'Sending data to Twilio', 'pronamic-ideal' ),
			],
			'gravityformswebhooks'         => [
				'active'                      => false,
				'meta_key_suffix'             => 'webhooks',
				'delayed_payment_integration' => false,
				'label'                       => __( 'Sending a trigger to Webhooks', 'pronamic-ideal' ),
			],
			'gravityformsdropbox'          => [
				'active'                      => false,
				'meta_key_suffix'             => 'dropbox',
				'delayed_payment_integration' => false,
				'label'                       => __( 'Uploading files to Dropbox', 'pronamic-ideal' ),
			],
			'gravityformszapier'           => [
				'active'                      => class_exists( 'GFZapier' ),
				'meta_key_suffix'             => 'zapier',
				'delayed_payment_integration' => false,
				'label'                       => __( 'Sending data to Zapier', 'pronamic-ideal' ),
				'delay_callback'              => function () {
			...
			'gravityformsuserregistration' => [
				...
				'delayed_payment_integration' => true,
				'label'                       => __( 'Registering the user', 'pronamic-ideal' ),
			],
			'gravityflow'                  => [
				'active'                      => false,
				'meta_key_suffix'             => 'gravityflow',
				'delayed_payment_integration' => true,
				'label'                       => __( 'Start the Workflow once payment has been received.', 'pronamic-ideal' ),
				'delay_callback'              => function () {
		...
		foreach ( $addons as $class ) {
			...
			if ( isset( $addon->delayed_payment_integration ) ) {
				...
				$actions[ $slug ]['label']                       = \sprintf(
					...
					\__( 'Process %s feeds', 'pronamic-ideal' ),
	...
	public function gform_payment_statuses( $payment_statuses ) {
		...
		$payment_statuses['PartlyRefunded'] = __( 'Partially Refunded', 'pronamic-ideal' );
