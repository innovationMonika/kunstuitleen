...
class PaymentMethodsField extends GF_Field_Select {
		...
		if ( is_admin() ) {
			...
			if ( empty( $feeds ) ) {
				$link = sprintf(
					...
					esc_html__( 'New Payment Feed', 'pronamic-ideal' )
				);
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Payment Method', 'pronamic-ideal' );
	}
	...
	public function get_form_editor_button() {
		return [
			...
			'text'  => __( 'Payment Method', 'pronamic-ideal' ),
		];
	}
	...
	public function editor_js_set_default_values() {
		...
		case '<?php echo esc_js( self::TYPE ); ?>' :
			if ( ! field.label ) {
				field.label = '<?php echo esc_js( __( 'Choose a payment method', 'pronamic-ideal' ) ); ?>';
			}
