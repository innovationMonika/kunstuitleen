...
class PaymentAddOn extends GFPaymentAddOn {
	...
	public function pronamic_maybe_save_feed() {
		...
		if ( '' === trim( $post_title ) ) {
			...
			$post_title = __( 'Payment Feed', 'pronamic-ideal' );
			...
			if ( $count > 1 ) {
				$post_title = sprintf(
					...
					__( 'Payment Feed #%d', 'pronamic-ideal' ),
					$count
				);
			}
		}
	...
	public function feed_list_title() {
		...
		if ( GravityForms::version_compare( '2.5-rc', '>=' ) ) {
			return __( 'Pay', 'pronamic-ideal' );
		}

		$title = sprintf(
			'<i class="dashicons dashicons-money fa-"></i> %s',
			esc_html__( 'Pay Feeds', 'pronamic-ideal' )
		);
		...
		$title .= sprintf(
			...
			esc_url( $url ),
			esc_html__( 'Add New', 'pronamic-ideal' )
		);
	...
	public function feed_list_columns() {
		return [
			'name'                    => esc_html__( 'Name', 'pronamic-ideal' ),
			'transaction_description' => esc_html__( 'Transaction Description', 'pronamic-ideal' ),
			'configuration'           => esc_html__( 'Configuration', 'pronamic-ideal' ),
		];
	}
	...
	public function get_column_value_name( $feed ) {
		...
		if ( empty( $title ) ) {
			$title = __( 'Default pay feed', 'pronamic-ideal' );
		}
	...
	public function feed_list_no_item_message() {
		...
		$label = \__( 'This form doesn’t have any pay feeds. Let’s go %1$screate one%2$s.', 'pronamic-ideal' );
	...
	public function supported_notification_events( $form ) {
		...
		$events = [
			'complete_payment'          => esc_html__( 'Payment Completed', 'pronamic-ideal' ),
			'fail_payment'              => esc_html__( 'Payment Failed', 'pronamic-ideal' ),
			'add_pending_payment'       => esc_html__( 'Payment Pending', 'pronamic-ideal' ),

			// Subscription events.
			'create_subscription'       => esc_html__( 'Subscription Created', 'pronamic-ideal' ),
			'cancel_subscription'       => esc_html__( 'Subscription Canceled', 'pronamic-ideal' ),
			'expire_subscription'       => esc_html__( 'Subscription Expired', 'pronamic-ideal' ),
			'renew_subscription'        => esc_html__( 'Subscription Renewal Notice', 'pronamic-ideal' ),
			'add_subscription_payment'  => esc_html__( 'Subscription Payment Added', 'pronamic-ideal' ),
			'fail_subscription_payment' => esc_html__( 'Subscription Payment Failed', 'pronamic-ideal' ),
		];
