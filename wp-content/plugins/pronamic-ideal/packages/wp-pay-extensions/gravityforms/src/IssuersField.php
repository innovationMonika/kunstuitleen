...
class IssuersField extends GF_Field_Select {
		...
		if ( is_admin() ) {
			...
			if ( empty( $feeds ) ) {
				$link = sprintf(
					...
					__( 'New Payment Feed', 'pronamic-ideal' )
				);
			...
			if ( ! empty( $feeds ) && empty( $this->choices ) ) {
				...
				$error = sprintf(
					...
					sprintf(
						...
						__( 'Please remove it from this form or <a href="%s" target="_blank">add a supported payment gateway</a>.', 'pronamic-ideal' ),
						esc_url( $new_feed_url )
					)
				);
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Issuer', 'pronamic-ideal' );
	}
	...
	public static function editor_js_set_default_values() {
		$label = __( 'Choose a bank for iDEAL payment', 'pronamic-ideal' );
