...
final class PaymentGateway extends NF_Abstracts_PaymentGateway {
	...
	public function __construct() {
		$this->_name = __( 'Pronamic Pay', 'pronamic-ideal' );
	...
	public function process( $action_settings, $form_id, $data ) {
		...
		try {
			...
			$data['actions']['success_message'] = __( 'Please wait while you are redirected to complete the payment.', 'pronamic-ideal' );
	...
	public function action_settings() {
		...
		// Configuration.
		$settings['config_id'] = [
			'label'   => __( 'Configuration', 'pronamic-ideal' ),
		...
		foreach ( Plugin::get_config_select_options() as $value => $label ) {
			if ( 0 === $value ) {
				$label = \__( '— Default Gateway —', 'pronamic-ideal' );
			}
		...
		$settings['pronamic_pay_status_pages'] = [
			...
			'type'     => 'fieldset',
			'label'    => __( 'Payment Status Pages', 'pronamic-ideal' ),
			'width'    => 'full',
			'group'    => 'pronamic_pay',
			'settings' => [],
		];
		...
		$options = [
			[
				'label' => __( '— Select —', 'pronamic-ideal' ),
			],
		];
		...
		// Add settings fields.
		$pages = [
			'completed' => \__( 'Completed', 'pronamic-ideal' ),
			'cancel'    => \__( 'Canceled', 'pronamic-ideal' ),
			'expired'   => \__( 'Expired', 'pronamic-ideal' ),
			'error'     => \__( 'Error', 'pronamic-ideal' ),
			'unknown'   => \__( 'Unknown', 'pronamic-ideal' ),
		];
		...
		if ( null !== $form_id ) {
			$settings['pronamic_pay_delayed_actions'] = [
				...
				'type'     => 'fieldset',
				'label'    => __( 'Delayed actions', 'pronamic-ideal' ),
				'width'    => 'full',
				'group'    => 'pronamic_pay',
				'settings' => [],
			];
