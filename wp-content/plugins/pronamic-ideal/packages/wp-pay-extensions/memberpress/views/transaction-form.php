...
<tr valign="top">
	<th scope="row">
		<label for="trans_num"><?php esc_html_e( 'Pronamic Payment', 'pronamic-ideal' ); ?></label>
	</th>
	<td>
		<?php
		...
		if ( \count( $items ) > 0 ) {
			...
			foreach ( $items as $payment_id => $url ) {
				...
				$status_label = isset( $status_object, $status_object->label ) ? $status_object->label : __( 'Unknown status', 'pronamic-ideal' );
