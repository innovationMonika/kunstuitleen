...
class Gateway extends WC_Payment_Gateway {
	...
	public function __construct( $args = [] ) {
		...
		if ( null === $this->method_title ) {
			$this->method_title = sprintf(
				...
				__( '%1$s - %2$s', 'pronamic-ideal' ),
				__( 'Pronamic', 'pronamic-ideal' ),
				PaymentMethods::get_name( $this->payment_method, __( 'Pronamic', 'pronamic-ideal' ) )
			);
		}
		...
		if ( null !== $this->payment_method ) {
			$this->order_button_text = sprintf(
				...
				__( 'Proceed to %s', 'pronamic-ideal' ),
				PaymentMethods::get_name( $this->payment_method, __( 'Pronamic', 'pronamic-ideal' ) )
			);
		}
	...
	public function init_form_fields() {
		...
		$this->form_fields = [
			'enabled'             => [
				'title'   => __( 'Enable/Disable', 'pronamic-ideal' ),
				'type'    => 'checkbox',
				'label'   => sprintf(
					/* translators: %s: payment method title */
					__( 'Enable %s', 'pronamic-ideal' ),
					$this->method_title
				),
				'default' => 'no',
			],
			'title'               => [
				'title'       => __( 'Title', 'pronamic-ideal' ),
				'type'        => 'text',
				'description' => $description_prefix . __( 'This controls the title which the user sees during checkout.', 'pronamic-ideal' ),
				'default'     => PaymentMethods::get_name( $this->payment_method, __( 'Pronamic', 'pronamic-ideal' ) ),
			],
			'description'         => [
				'title'       => __( 'Description', 'pronamic-ideal' ),
				'type'        => 'textarea',
			...
			'icon_display'        => [
				'title'       => __( 'Icon display', 'pronamic-ideal' ),
				'type'        => 'select',
				'options'     => [
					'default' => \__( 'Default', 'pronamic-ideal' ),
					'none'    => \__( 'None', 'pronamic-ideal' ),
					'custom'  => \__( 'Custom', 'pronamic-ideal' ),
				],
				'description' => sprintf(
					'%s%s',
					$description_prefix,
					__( 'This controls how the payment gateway icon is displayed on the checkout page.', 'pronamic-ideal' )
				),
				'default'     => ( '' === $icon_display && '' !== $icon_url ) ? 'custom' : '',
			],
			'icon'                => [
				'title'       => __( 'Custom icon URL', 'pronamic-ideal' ),
				'type'        => 'text',
				'description' => sprintf(
					'%s%s',
					$description_prefix,
					__( 'Provide a publicly accessible URL for your own payment gateway icon (only used if "Custom" is selected in the icon display setting).', 'pronamic-ideal' )
				),
				'default'     => '',
			],
			'config_id'           => [
				'title'   => __( 'Configuration', 'pronamic-ideal' ),
				'type'    => 'select',
				'default' => get_option( 'pronamic_pay_config_id' ),
				'options' => Plugin::get_config_select_options( $this->payment_method ),
			],
			'payment'             => [
				'title'       => __( 'Payment Options', 'pronamic-ideal' ),
				'type'        => 'title',
				'description' => '',
				'default'     => '',
			],
			'payment_description' => [
				'title'       => __( 'Payment Description', 'pronamic-ideal' ),
				'type'        => 'text',
				'description' => sprintf(
					'%s%s<br />%s<br />%s',
					$description_prefix,
					__( 'This controls the payment description.', 'pronamic-ideal' ),
					/* translators: %s: default code */
					sprintf( __( 'Default: <code>%s</code>', 'pronamic-ideal' ), __( 'Order {order_number}', 'pronamic-ideal' ) ),
					/* translators: %s: tags */
					sprintf( __( 'Tags: %s', 'pronamic-ideal' ), sprintf( '<code>%s</code> <code>%s</code> <code>%s</code>', '{order_number}', '{order_date}', '{payment_lines_name}', '{blogname}' ) )
				),
				'default'     => __( 'Order {order_number}', 'pronamic-ideal' ),
			],
		];
		...
		if ( isset( $this->gateway_args['icon'] ) ) {
			...
			$this->form_fields['icon']['description'] = sprintf(
				'%s%s<br />%s',
				$description_prefix,
				__( 'This controls the icon which the user sees during checkout.', 'pronamic-ideal' ),
				/* translators: %s: default code */
				sprintf( __( 'Default: <code>%s</code>', 'pronamic-ideal' ), $this->form_fields['icon']['default'] )
			);
		}
	...
	public function process_payment( $order_id ) {
		...
		if ( null === $gateway ) {
			$notice = __( 'The payment gateway could not be found.', 'pronamic-ideal' );
			...
			if ( current_user_can( 'manage_options' ) && empty( $this->config_id ) ) {
				...
				$notice = sprintf(
					...
					__( 'You have to select a gateway configuration on the <a href="%s">WooCommerce checkout settings page</a>.', 'pronamic-ideal' ),
					add_query_arg(
						[
							'page'    => 'wc-settings',
							'tab'     => 'checkout',
	...
	private function new_pronamic_payment_from_wc_order( WC_Order $order ) {
		...
		$title = sprintf(
			...
			sprintf(
				...
				__( 'WooCommerce order %s', 'pronamic-ideal' ),
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( null === $gateway ) {
			return new \WP_Error(
				...
				__( 'Unable to process refund as gateway configuration does not exist.', 'pronamic-ideal' )
			);
		}
		...
		if ( null === $payment ) {
			return new \WP_Error(
				...
				\__( 'Cannot process refund because payment could not be found.', 'pronamic-ideal' )
			);
		}
		...
		try {
			...
			$note = \sprintf(
				...
				\__( 'Created refund of %1$s with reference `%2$s`.', 'pronamic-ideal' ),
	...
	public function after_checkout_validation( $data, $errors ) {
		...
		foreach ( $input_ids as $input_id ) {
			...
			if ( empty( $input_value ) ) {
				$error = sprintf(
					/* translators: %s: payment method title */
					__( 'A required field for the %s payment method is empty.', 'pronamic-ideal' ),
					$this->method_title
				);
