...
class Extension extends AbstractPluginIntegration {
	...
	public static function get_gateways() {
		...
		$gateways = [
			[
				'id'                 => 'pronamic_pay',
				'payment_method'     => null,
				'method_title'       => __( 'Pronamic', 'pronamic-ideal' ),
				'method_description' => __( "This payment method does not use a predefined payment method for the payment. Some payment providers list all activated payment methods for your account to choose from. Use payment method specific gateways (such as 'iDEAL') to let customers choose their desired payment method at checkout.", 'pronamic-ideal' ),
				'check_active'       => false,
			],
		];
	...
	public static function woocommerce_thankyou_order_received_text( $message, $order ) {
		...
		// Add notice.
		$message .= ' ' . \__( 'We process your order as soon as we have processed your payment.', 'pronamic-ideal' );
	...
	public static function reservation_cancelled_note( Payment $payment ) {
		...
		$order->add_order_note(
			sprintf(
				...
				__( 'reserved payment cancelled', 'pronamic-ideal' )
			)
		);
	}
	...
	public static function status_update( Payment $payment ) {
		...
		$note = sprintf(
			...
			__( '<a href="%1$s">Payment #%2$s</a> via "%3$s" updated to "%4$s".', 'pronamic-ideal' ),
	...
	public static function admin_init() {
		...
		add_settings_field(
			...
			'pronamic_pay_woocommerce',
			[
				'legend'      => __( 'Add date of birth field', 'pronamic-ideal' ),
				'description' => __( 'Add date of birth field to billing checkout fields', 'pronamic-ideal' ),
		...
		add_settings_field(
			...
			'pronamic_pay_woocommerce',
			[
				'legend'      => __( 'Add gender field', 'pronamic-ideal' ),
				'description' => __( 'Add gender field to billing checkout fields', 'pronamic-ideal' ),
	...
	public static function input_checkout_fields_select( $args ) {
		...
		$placeholder_option->children[] = \__( '— Select a checkout field —', 'pronamic-ideal' );
	...
	public static function checkout_fields( $fields ) {
		...
		if ( $enable_birth_date_field ) {
			$fields['billing']['pronamic_pay_birth_date'] = [
				'type'     => 'date',
				'label'    => __( 'Date of birth', 'pronamic-ideal' ),
				'priority' => 110,
			];
		}
		...
		if ( $enable_gender_field ) {
			$fields['billing']['pronamic_pay_gender'] = [
				'type'     => 'select',
				'label'    => __( 'Gender', 'pronamic-ideal' ),
				'priority' => 120,
				'options'  => [
					''  => __( '— Select gender —', 'pronamic-ideal' ),
					'F' => __( 'Female', 'pronamic-ideal' ),
					'M' => __( 'Male', 'pronamic-ideal' ),
					'X' => __( 'Other', 'pronamic-ideal' ),
				],
			];
		}
	...
	public static function source_description( $description, Payment $payment ) {
		return __( 'WooCommerce Order', 'pronamic-ideal' );
	}
