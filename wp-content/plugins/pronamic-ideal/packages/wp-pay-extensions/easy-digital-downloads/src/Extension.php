...
class Extension extends AbstractPluginIntegration {
	...
	public function plugins_loaded() {
		...
		new Gateway(
			[
				'id'             => 'pronamic_ideal',
				'admin_label'    => __( 'Pronamic', 'pronamic-ideal' ),
				'checkout_label' => __( 'iDEAL', 'pronamic-ideal' ),
			]
		);
		...
		if ( $should_update ) {
			switch ( $payment->get_status() ) {
				...
				case Core_Statuses::FAILURE:
					...
					break;
				case Core_Statuses::SUCCESS:
					\edd_insert_payment_note( $source_id, __( 'Payment completed.', 'pronamic-ideal' ) );
					...
					break;
				case Core_Statuses::OPEN:
					\edd_insert_payment_note( $source_id, __( 'Payment open.', 'pronamic-ideal' ) );

					break;
				default:
					\edd_insert_payment_note( $source_id, __( 'Payment unknown.', 'pronamic-ideal' ) );
	...
	public function source_text( $text, Payment $payment ) {
		$text = \__( 'Easy Digital Downloads', 'pronamic-ideal' ) . '<br />';

		$text .= \sprintf(
			'<a href="%s">%s</a>',
			EasyDigitalDownloads::get_payment_url( $payment->get_source_id() ),
			/* translators: %s: payment number */
			\sprintf( __( 'Payment %s', 'pronamic-ideal' ), $payment->get_source_id() )
		);
	...
	public function source_description( $description, Payment $payment ) {
		return \__( 'Easy Digital Downloads Order', 'pronamic-ideal' );
	}
	...
	public static function edd_payment_statuses( $payment_statuses ) {
		if ( ! array_key_exists( 'cancelled', $payment_statuses ) ) {
			$payment_statuses['cancelled'] = __( 'Cancelled', 'pronamic-ideal' );
		}

		if ( ! array_key_exists( 'partially_refunded', $payment_statuses ) ) {
			$payment_statuses['partially_refunded'] = __( 'Partially Refunded', 'pronamic-ideal' );
		}
	...
	private function register_post_statuses() {
		register_post_status(
			'cancelled',
			[
				'label'                     => _x( 'Cancelled', 'Easy Digital Downloads cancelled payment status', 'pronamic-ideal' ),
				'public'                    => true,
		...
		register_post_status(
			'partially_refunded',
			[
				'label'                     => _x( 'Partially Refunded', 'Easy Digital Downloads payment status', 'pronamic-ideal' ),
				'public'                    => true,
				'exclude_from_search'       => false,
				'show_in_admin_all_list'    => true,
				'show_in_admin_status_list' => true,
				/* translators: %s: count value */
				'label_count'               => _n_noop( 'Partially Refunded <span class="count">(%s)</span>', 'Partially Refunded <span class="count">(%s)</span>', 'pronamic-ideal' ),
			]
		);
	}
