...
class Gateway {
	...
	public function settings_gateways( $settings_gateways ) {
		$settings = [
			$this->id                     => [
				...
				'name' => '<strong>' . \sprintf( __( '%s Settings', 'pronamic-ideal' ), $this->admin_label ) . '</strong>',
				/* translators: %s: gateway admin label */
				'desc' => \sprintf( __( 'Configure the %s settings', 'pronamic-ideal' ), $this->admin_label ),
				'type' => 'header',
			],
			$this->id . '_config_id'      => [
				'id'      => $this->id . '_config_id',
				'name'    => __( 'Gateway Configuration', 'pronamic-ideal' ),
				'type'    => 'select',
			...
			$this->id . '_checkout_label' => [
				...
				'name' => __( 'Checkout Label', 'pronamic-ideal' ),
				'type' => 'text',
			...
			$this->id . '_description'    => [
				...
				'name' => __( 'Description', 'pronamic-ideal' ),
				'type' => 'text',
				// @link https://github.com/easydigitaldownloads/Easy-Digital-Downloads/blob/2.5.9/includes/admin/settings/register-settings.php#L1537-L1541
				// @link https://github.com/easydigitaldownloads/Easy-Digital-Downloads/blob/2.5.9/includes/gateways/amazon-payments.php#L330
				'std'  => '{edd_cart_details_name}',
				/* translators: %s: <code>{edd_cart_details_name}</code> */
				'desc' => '<br />' . \sprintf( __( 'Default: %s', 'pronamic-ideal' ), '<code>{edd_cart_details_name}</code>' ) .
					/* translators: %s: <code>{edd_cart_details_name}</code> */
					'<br />' . \sprintf( __( 'Available Tags: %s', 'pronamic-ideal' ), '<code>{edd_cart_details_name}</code> <code>{edd_payment_id}</code>' ),
			],
		];
	...
	public function process_purchase( $purchase_data ) {
		...
		if ( false === $edd_payment_id ) {
			...
			\edd_record_gateway_error(
				...
				\sprintf(
					...
					\__( 'Payment creation failed before sending buyer to the payment provider. Payment data: %s', 'pronamic-ideal' ),
		...
		$payment->title    = sprintf(
			...
			__( 'Easy Digital Downloads order %s', 'pronamic-ideal' ),
			$payment->order_id
		);
		...
		$note = sprintf(
			...
			__( 'Payment %s pending.', 'pronamic-ideal' ),
