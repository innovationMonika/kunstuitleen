			...
			if ( false === $dry_run ) {
				...
				$subscription->add_note(
					\sprintf(
						...
						__( 'Since Restrict Content Pro 3 a subscription must be linked to a Restrict Content Pro membership. That\'s why source "%1$s" with ID "%2$s" was updated to source "%3$s" with ID "%4$s".', 'pronamic-ideal' ),
						$subscription_source,
						$subscription_source_id,
						'rcp_membership',
						$rcp_membership->get_id()
	...
	private function upgrade_payments() {
		...
		foreach ( $payment_posts as $payment_post ) {
			...
			$payment->add_note(
				\sprintf(
					/* translators: 1: Old source, 2: Old source ID, 3: New source, 4: New source ID. */
					__( 'Since Restrict Content Pro 3 a payment must be linked to a Restrict Content Pro payment. That\'s why source "%1$s" with ID "%2$s" was updated to source "%3$s" with ID %4$s".', 'pronamic-ideal' ),
					$payment_source,
					$payment_source_id,
					'rcp_payment',
					$rcp_payment->id
