...
class Extension extends AbstractPluginIntegration {
	...
	public function __construct( $args = [] ) {
		$args = wp_parse_args(
			$args,
			[
				'name'                => \__( 'Restrict Content Pro', 'pronamic-ideal' ),
		...
		switch ( $new_status ) {
			case MembershipStatus::ACTIVE:
				$note = sprintf(
					...
					__( 'Subscription activated by %s.', 'pronamic-ideal' ),
					__( 'Restrict Content Pro', 'pronamic-ideal' )
				);
			...
			case MembershipStatus::CANCELLED:
				$note = sprintf(
					/* translators: %s: Restrict Content Pro */
					__( 'Subscription canceled by %s.', 'pronamic-ideal' ),
					__( 'Restrict Content Pro', 'pronamic-ideal' )
				);
			...
			case MembershipStatus::EXPIRED:
				$note = sprintf(
					/* translators: %s: Restrict Content Pro */
					__( 'Subscription expired by %s.', 'pronamic-ideal' ),
					__( 'Restrict Content Pro', 'pronamic-ideal' )
				);
			...
			case MembershipStatus::PENDING:
				$note = sprintf(
					/* translators: %s: Restrict Content Pro */
					__( 'Subscription pending by %s.', 'pronamic-ideal' ),
					__( 'Restrict Content Pro', 'pronamic-ideal' )
				);
	...
	public function rcp_subscription_details_actions( $actions, $links, $user_id, $rcp_membership ) {
		...
		$action = \sprintf(
			...
			\esc_attr( \__( 'Update payment method', 'pronamic-ideal' ) ),
			\esc_html( \__( 'Update payment method', 'pronamic-ideal' ) )
		);
	...
	public function payment_source_text( $text, Payment $payment ) {
		$text = __( 'Restrict Content Pro', 'pronamic-ideal' ) . '<br />';
		...
		$text .= sprintf(
			...
			sprintf( __( 'Payment %s', 'pronamic-ideal' ), $payment->source_id )
		);
	...
	public function payment_source_description( $description, Payment $payment ) {
		switch ( $payment->source ) {
			case 'rcp_payment':
				return __( 'Restrict Content Pro Payment', 'pronamic-ideal' );
			default:
				return $description;
		}
	}
	...
	public function subscription_source_description( $description, Subscription $subscription ) {
		switch ( $subscription->get_source() ) {
			case 'rcp_membership':
				return __( 'Restrict Content Pro Membership', 'pronamic-ideal' );
			default:
				return $description;
		}
	}
	...
	public function subscription_source_text( $text, Subscription $subscription ) {
		...
		$text .= sprintf(
			...
			sprintf( __( 'Membership %s', 'pronamic-ideal' ), $subscription->get_source_id() )
		);
