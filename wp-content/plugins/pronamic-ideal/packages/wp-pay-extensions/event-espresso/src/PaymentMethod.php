...
class PaymentMethod extends EE_PMT_Base {
	...
	public function __construct( $pm_instance = null ) {
		if ( null === $this->payment_method ) {
			...
			$this->_pretty_name = __( 'Pronamic', 'pronamic-ideal' );
		}
			...
			[
				'extra_meta_inputs' => [
					'config_id'               => new EE_Select_Input(
						$config_options,
						[
							'html_label_text' => __( 'Configuration', 'pronamic-ideal' ),
							'default'         => get_option( 'pronamic_pay_config_id' ),
						]
					),
					'transaction_description' => new EE_Text_Input(
						[
							'html_label_text' => __( 'Transaction description', 'pronamic-ideal' ),
							/* translators: %s: <code>{tag}</code> */
							'html_help_text'  => sprintf( __( 'Available tags: %s', 'pronamic-ideal' ), sprintf( '<code>%s</code>', '{transaction_id}' ) ),
							'default'         => __( 'Event Espresso transaction {transaction_id}', 'pronamic-ideal' ),
						]
					),
				],
			]
