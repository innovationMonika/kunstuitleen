...
class Gateway {
	...
	public function gateways_sections( $sections ) {
		// Section title.
		$title = \__( 'Pronamic', 'pronamic-ideal' );
	...
	public function gateway_settings( $settings ) {
		...
		$settings[] = [
			'name'    => __( 'Configuration', 'pronamic-ideal' ),
			'desc'    => '',
		...
		$settings[] = [
			'name'    => __( 'Transaction description', 'pronamic-ideal' ),
			'desc'    => sprintf(
			...
			'type'    => 'text',
			'default' => __( 'Give donation {donation_id}', 'pronamic-ideal' ),
		];
	...
	public function process_purchase( $purchase_data ) {
		if ( ! wp_verify_nonce( $purchase_data['gateway_nonce'], 'give-gateway' ) ) {
			wp_die( esc_html__( 'Nonce verification has failed', 'pronamic-ideal' ), esc_html__( 'Error', 'pronamic-ideal' ), [ 'response' => 403 ] );
		}
		...
		if ( false === $donation_id ) {
			...
			give_record_gateway_error(
				__( 'Payment Error', 'pronamic-ideal' ),
				sprintf(
					/* translators: %s: payment data as JSON */
					__( 'Payment creation failed before sending buyer to payment provider. Payment data: %s', 'pronamic-ideal' ),
