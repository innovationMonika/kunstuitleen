...
params="id="+viewId;if(false!==comment)params+="&frm_view_page_state="+encodeURIComponent(getStateFromComment(comment));if("__FRMTABLES"in window&&undefined!==__FRMTABLES.graphs)params+="&frm_graph_offset="+__FRMTABLES.graphs.length;params+="&frm_modal_offset="+getModalOffset();xhr.send(params);function handleReadyStateChange(){var json,newGraphData;if(!requestIsSuccessful(xhr))return;json=JSON.parse(xhr.responseText);callback(json.html);if(undefined!==json.graphs){if("__FRMTABLES"in window){newGraphData=
JSON.parse(JSON.stringify(window.__FRMTABLES));if(undefined!==json.graphs.graphs)each(json.graphs.graphs,function(graph){newGraphData.graphs.push(graph)})}else newGraphData=json.graphs;__FRMTABLES=newGraphData;frmProForm.loadGoogle()}if("string"===typeof json.modal&&""!==json.modal)document.body.insertAdjacentHTML("beforeend",json.modal)}}function getModalOffset(){var numberOfModals=0;while(document.getElementById("frm-modal-"+numberOfModals))++numberOfModals;return numberOfModals}function setQueryStringParameter(name,
value){if("function"!==typeof URLSearchParams)return;const params=new URLSearchParams(window.location.search);params.set(name,value);window.history.replaceState({},"",decodeURIComponent(`${window.location.pathname}?${params}`))}function getStateFromComment(comment){var text,start,end;text=comment.nodeValue;start=text.indexOf('state="');if(-1===start)return"";start+=7;end=text.indexOf('"',start+1);return text.substring(start,end)}function requestIsSuccessful(xhr){return xhr.readyState>3&&xhr.status==
200&&xhr.responseText!==""}function loadInfiniteScroll(trigger){var viewId,page,paginationContainer,comment;if(hasClass(trigger,"frm-loading-infinite-scroll"))return;viewId=parseInt(trigger.getAttribute("frm-view-id"));page=parseInt(trigger.getAttribute("frm-page"))+1;paginationContainer=getPaginationContainerFromChild(trigger);trigger.className+=" frm-loading-infinite-scroll";if(handleSpecialInfiniteScrollType(trigger,viewId,page,paginationContainer))return;comment=loopPreviousSiblingsUntilComment(paginationContainer);
...
function(element){return hasClass(element,"frm-grid-view")},".frm-grid-view")}function checkBeforePaginationContainerForMatch(paginationContainer,callback){var match=false;loopPreviousSiblingsUntilComment(paginationContainer,function(previousSibling){if(callback(previousSibling)){match=previousSibling;return false}});return match}function checkWrapperForComment(wrapper){var element=wrapper.firstChild;return checkElementForComment(element)}function checkElementForComment(element){if(!element)return false;
if("P"===element.nodeName)return checkParagraphForViewComment(element);if(nodeIsViewComment(element))return element;return false}function replaceComment(newComment,oldComment){if(newComment)oldComment.parentNode.insertBefore(newComment,oldComment);maybeRemove(oldComment)}function loadInfiniteScrollForGrid(args){var grid,comment;grid=checkBeforePaginationContainerForGrid(args.paginationContainer);if(false===grid)return;comment=checkForViewComment(grid,args.paginationContainer);loadPage(args.viewId,
args.page,comment,function(html){var wrapper=createWrapperWithReplacedCommentAndTrigger(html,comment,args.trigger);each(nodeListToArray(wrapper.querySelector(".frm-grid-view").children),function(row){grid.appendChild(row)})})}function loadInfiniteScrollForContainer(args){var container,comment;container=args.paginationContainer.previousElementSibling;comment=loopPreviousSiblingsUntilComment(container);loadPage(args.viewId,args.page,comment,function(html){var wrapper=createWrapperWithReplacedCommentAndTrigger(html,
comment,args.trigger);each(nodeListToArray(wrapper.childNodes),function(child){if(!hasClass(child,"frm_pagination_cont"))container.appendChild(child)})})}function hasClass(element,targetClass){var className=" "+element.className+" ";return-1!==className.indexOf(" "+targetClass+" ")}function getClosestParent(element,selector){var activeParent=element.parentNode;while(activeParent&&"function"===typeof activeParent.matches){if(activeParent.matches(selector))return activeParent;activeParent=activeParent.parentNode}return false}
function each(items,callback){var index,length;length=items.length;for(index=0;index<length;index++)if(false===callback(items[index],index))break}})();
